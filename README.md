...
Este é um projeto pessoal desenvolvido em C++ com o objetivo de aprofundar meus conhecimentos na linguagem. O foco principal é a exploração e utilização exclusiva das bibliotecas padrões e métodos da própia linguagem, sem dependências de frameworks ou bibliotecas externas.
...
*   **Classes, Structs e Enums:** `CamelCase` (PascalCase), iniciando com a letra maiúscula.
*   **Funções e Métodos:** `camelCase` (lowerCamelCase), iniciando com a letra minúscula.
*   **Variáveis Locais e Parâmetros:** `camelCase` ou `snake_case`.
*   **Variáveis Membro (Classes):** `snake_case_` (com um underscore no final).
*   **Constantes e Macros:** `ALL_CAPS_SNAKE_CASE` (letras maiúsculas separadas por underscores).
*   **Namespaces:** `snake_case` inteiramente em minúsculo.
