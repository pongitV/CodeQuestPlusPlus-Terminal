...
std::vector<std::string> Combate3DRenderer::renderizarQuadro(
    ...
    std::vector<std::string> tela = Raycaster::desenharQuadroEstatico3D(arena, jX, jY, angulo, tituloMapa, jogador, altura3D);
...
void Combate3DRenderer::sobreporSprite(
    ...
    auto texturizar = [&](char c, int rx, int ry) -> std::string {
        ...
        // Efeito de Sangue em low HP (<= 30% HP)
        if (inimigo->obterVida() > 0 && inimigo->obterVida() <= inimigo->obterVidaMaxima() * 0.3f) {
            if ((rx + ry * 3) % 7 == 0) {
                rMod = 180; gMod = 0; bMod = 0; // Sangue vermelho escuro
            }
        }
    ...
    // Cálculo da dissolução da morte (de cima para baixo)
    int totalLinhas2D = static_cast<int>(inimigo->obterRaca()->obterAparenciaCombate().size());
    if (totalLinhas2D <= 0) totalLinhas2D = 1;
    int linhasParaApagar3D = isMorte ? (frameMorte * alturaArte) / totalLinhas2D : 0;

    // Desenha contorno preto (borda do sprite) + corpo texturizado
    for (int y = 0; y < alturaArte; y++) {
        // Se estiver na animação de morte, apaga as linhas superiores gradualmente
        if (isMorte && y < linhasParaApagar3D) {
            continue;
        }
    ...
    if (isMorte && frameMorte >= totalLinhas2D && !dropsAnimacao.empty()) {
