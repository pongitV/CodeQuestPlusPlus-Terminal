...
protected:
    ...
    int precoVenda;
    std::function<void(Personagem*, Personagem*)> acaoUsar;
    std::function<bool(Item*, Personagem*, bool*)> acaoInventario;
...
public:
    ...
    virtual void usar(Personagem* usuario, Personagem* alvo) {
        if (usuario == nullptr || alvo == nullptr) return;
        if (acaoUsar) acaoUsar(usuario, alvo);
    }
    virtual void definirAcaoUsar(std::function<void(Personagem*, Personagem*)> acao) { acaoUsar = acao; }
    
    virtual void definirAcaoInventario(std::function<bool(Item*, Personagem*, bool*)> acao) { acaoInventario = acao; }
    virtual bool usarDoInventario(Personagem* usuario, bool* turnoFoiConsumido) {
