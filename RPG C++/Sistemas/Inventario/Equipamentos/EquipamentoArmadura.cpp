...
bool EquipamentoArmadura::podeSerEquipadoPor(Personagem* personagem) const {
    if (!personagem) return false;
    return personagem->obterResistencia() >= reqResistencia &&
           personagem->obterConstituicao() >= reqConstituicao;
}

std::vector<std::string> EquipamentoArmadura::obterDetalhesInspecao(Personagem* personagem) const {
