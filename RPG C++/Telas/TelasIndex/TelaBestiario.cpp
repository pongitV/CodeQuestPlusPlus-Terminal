...
#include "Telas/TelaBase.h"
#include "Telas/TelasIndex/TelaBestiarioLayouts.h"
#include "Telas/TelasIndex/TelaRacasLayouts.h"

static void exibirCabecalho(int largura, const std::string& tituloSecundario = "", bool animar = false, bool modoRaca = false) {
    Aparencia::limparTela();
    if (modoRaca) {
        Aparencia::exibirLogoAscii(ArtesRacas::logoRacas, 66, Cor::AMARELO, tituloSecundario, animar);
    } else {
    ...
    while (true) {
        ...
        try {
            ...
            if (escolha >= 1 && escolha <= indiceFinal) {
                exibirFicha(jogadorAtual, descobertos[escolha - 1]);
            }
...
void TelaBestiario::exibirFicha(SistemaPersonagem* jogadorAtual, const std::string& nomeSelecionado, bool modoRaca) {
    if (jogadorAtual == nullptr) return;
    ...
    while (true) {
        exibirCabecalho(largura, "FICHA DO INIMIGO", false, modoRaca);
