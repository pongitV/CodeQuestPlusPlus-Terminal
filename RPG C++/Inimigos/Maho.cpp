#include "Maho.h"
#include <iostream>
#include "../Sistemas/SistemaPersonagem.h"
#include "../Inventario/FabricaItens.h"
#include "../Utilidades/Aparencia.h"
#include "../Gerenciadores/GerenciadorDrops.h"
#include "../Utilidades/GeradorAleatorio.h"
#include "../Telas/TelaCombate.h"
#include <memory>

// --- INFORMACOES DA RACA ---
std::string Maho::obterNomeRaca() const { 
    return "General Divino Imoral da Espada de Oito Empunhaduras, Mahoraga"; 
}

Atributos Maho::obterAtributosRaca() const { 
    return { 1000, 0, 0, 0, 0, 0, 0 }; 
}

std::vector<std::unique_ptr<Item>> Maho::obterEquipamentoRaca() const {
    return {};
}

// --- HABILIDADE DA RACA ---
std::string Maho::obterNomeHabilidadeRaca() const { return "A Roda da Adaptacao"; }
std::string Maho::obterDescricaoHabilidadeRaca() const { return "Adapta-se ao alvo. Apos sofrer 10 parrys perfeitos, torna-se Imparavel."; }

// --- MECANICA DE ADAPTACAO ---
void Maho::aoCausarDano(SistemaPersonagem* atacante, SistemaPersonagem* alvo, int danoCausado) {
    // Efeito popup de texto piscante
    TelaCombate::adicionarMensagemFixa(Aparencia::margemCombate() + "\033[5m" + Aparencia::cor(Cor::AMARELO) + "* KLINK! *" + Aparencia::cor(Cor::RESET) + " A Roda gira...\n");

    // Adaptacao de Defesa
    int forcaFisica = alvo->obterForca() + alvo->obterDestreza();
    int forcaMagica = alvo->obterInteligencia() + alvo->obterSabedoria();
    
    if (forcaFisica >= forcaMagica) {
        if (atacante->obterResistencia() < 50) {
            atacante->alterarAtributoEstatico(TipoAtributo::Forca, 5);
            atacante->alterarAtributoEstatico(TipoAtributo::Destreza, 5);
            atacante->alterarAtributoEstatico(TipoAtributo::Resistencia, 5);
            atacante->alterarAtributoEstatico(TipoAtributo::Constituicao, 5);
            Aparencia::registrarLogBatalha(Aparencia::cor(Cor::MAGENTA) + ">> A Roda gira... Mahoraga adaptou-se ao combate fisico, ganhando atributos ofensivos e defensivos!" + Aparencia::cor(Cor::RESET));
        } else {
            Aparencia::registrarLogBatalha(Aparencia::cor(Cor::MAGENTA) + ">> A Roda gira... A adaptacao fisica de Mahoraga atingiu o limite de 50!" + Aparencia::cor(Cor::RESET));
        }
    } else {
        if (atacante->obterSabedoria() < 50) {
            atacante->alterarAtributoEstatico(TipoAtributo::Inteligencia, 5);
            atacante->alterarAtributoEstatico(TipoAtributo::Destreza, 5);
            atacante->alterarAtributoEstatico(TipoAtributo::Sabedoria, 5);
            atacante->alterarAtributoEstatico(TipoAtributo::Constituicao, 5);
            Aparencia::registrarLogBatalha(Aparencia::cor(Cor::MAGENTA) + ">> A Roda gira... Mahoraga adaptou-se a sua magia, ganhando atributos magicos e vitais!" + Aparencia::cor(Cor::RESET));
        } else {
            Aparencia::registrarLogBatalha(Aparencia::cor(Cor::MAGENTA) + ">> A Roda gira... A adaptacao magica de Mahoraga atingiu o limite de 50!" + Aparencia::cor(Cor::RESET));
        }
    }

    // Cura
    int cura = atacante->obterVidaMaxima() * 0.05; 
    atacante->modificarVida(cura);
    Aparencia::registrarLogBatalha(Aparencia::cor(Cor::VERDE) + ">> MAHO regenerou " + std::to_string(cura) + " HP" + Aparencia::cor(Cor::RESET));

    // Limpeza de debuffs
    std::vector<EfeitoID> efeitos;
    atacante->obterIDsEfeitosAtivos(efeitos);
    if (!efeitos.empty()) {
        atacante->limparEfeitos();
        Aparencia::registrarLogBatalha(Aparencia::cor(Cor::BRANCO) + ">> Mahoraga adaptou-se a todos os status negativos" + Aparencia::cor(Cor::RESET));
    }
}

void Maho::aoSofrerParryPerfeito() {
    parrysSofridos++;
    if (parrysSofridos == 10) {
        TelaCombate::adicionarMensagemFixa(Aparencia::margemCombate() + "\033[5m" + Aparencia::cor(Cor::AMARELO) + "* KLINK! *" + Aparencia::cor(Cor::RESET) + " A Roda gira...\n");
        Aparencia::registrarLogBatalha(Aparencia::cor(Cor::MAGENTA) + ">> A Roda gira... Mahoraga adaptou-se aos seus reflexos! Seus ataques agora sao IMPARAVEIS!" + Aparencia::cor(Cor::RESET));
    }
}

bool Maho::ignoraParry() const {
    return parrysSofridos >= 10;
}

// --- APARENCIA ---
const std::vector<std::string>& Maho::obterAparenciaRaca() const
{
    static std::vector<std::string> aparenciaReduzida;
    if (aparenciaReduzida.empty())
    {
        static const std::vector<std::string> aparenciaOriginal =
        {
        "......                                                                      ",
        "                                                                                   .::...::-                                                                     ",
        "                                                                                  .--::..:+*+               ..                                                   ",
        "                                                      ...:.:.                     .----:*****            ::::.:::                                                ",
        "                                                     :::..::-*                     =:*#*****=           --::...:-*                                               ",
        "                                                    :--:::-***=              ....::*+#####*            .---::::***:                                              ",
        "                                                    :---+*-=##    .-=++********=-*******+- ..       .:-=----+*****                                               ",
        "                                                     ***###=+************+=-..:*.....:=++****:...:***-..+********-                                               ",
        "                                                     .-++****::*:..........:=*.............-+*=...... ....=****-                                                 ",
        "                             ....:.               :++*****-...-----::---.-+*.........:+*+:......=. . .........                                                   ",
        "                            :::..::-*         .=+****-........----:::---=*..:::-+*+:.. . ......=..    .::::::::.                                                 ",
        "                           .:::..:=***      .+****...........:----:.:--####**:...............:**......---:...:-*-                                                ",
        "                           :-:::+*****++++=+*=-------------======----**##=--::::::::::::::::*-:::::-------:::****                                                ",
        "                            :=*******.   =+*-....... .....:--=====+**::*##...............:***        ------*****+                                                ",
        "                             =*****#   .=++............-+*=....-*#####::+:... ........=**++           =*********                                                 ",
        "                                       .:+.......-++-.. .....-+.**#####+::........:***+-               .%*****.                                                  ",
        "                      .:::::.         ....:=**:...... ....:++............+---=******.               #-.++                                                        ",
        "                     :::.:::--   .:::-**.::::...........:*=........::..-***--=---:::              .+=#*:                                                         ",
        "                    :::...:-***=*+        .:----------:::--====+********#+ ----::::---          :=====                                                           ",
        "                    --::::*****:           .....:-=::*=-*****+++=.        .---::.::=**        .===::=                                                            ",
        "                    .:-=*******          :::::----=*:...=..               :---:::=****.     +=++.=.                                                              ",
        "                     .*******+        =:--::.::--*+:..:--:.=               ----*******    **+-.+                                                                 ",
        "                        .:.            :--::.::****-=.=-..-:                *********   +#=-:+                                                                   ",
        "                                       :---::******:%:#..:..                  -++*-  -*===.=                                                                     ",
        "                                        :-********=-:.:===                          .==#+.+                                                                      ",
        "                                         .*******:.#           -....=.............+====..+                                                                       ",
        "                                           +.:...=:=        +.....=.............+==-...:+                                                                        ",
        "                                            ..*:----.    .:..........:..:===++-==..==-=+=.::                                                                     ",
        "                                            -.-..:-:=   :......-....-.:+:=**=*.:=========+-..:                                                                   ",
        "                                             ....:.++.  .:.-..:..:+..=+.===+.-*=========+======-.                                                                ",
        "                                             :.+..:--:-.........+=.-==.+==.-*===================*=                                                               ",
        "                                           .=*...::=*. ...=-.-.+:.===:+=.:*===============+=====*==                                                              ",
        "                                         :=:=-...:--*:..=*=..--=====+=.:+===+==-...======+======*===                                                             ",
        "                                       -.-*:%#@*+===..=..-:..:+..++...=========...=:..+===+=====+===+   ..:=.+===. ..                                            ",
        "                                     ..:...=-:--+:*=..-=:..:=:---=--=+=+==*=++-=:*=+.::==+-.+=++=*=*+*=**========+=-                                             ",
        "                                   .:..:*-.......-++*-::=-..=:+=...#=+=*+==================*==**==*===#====+*===+.                                               ",
        "                                 .------==:...::.:-:.:-+=-:-======..=====+========+=========+==+==+-..:-++=-:..                                                  ",
        "                                            ..-..==+.......+-=+::==*....:.....::-+==-:.......:-:..:=......-:                                                     ",
        "                                          .........-........+=*==-=#==*++=====...=+.......:-+...-.............-                                                  ",
        "                                        .-........:....:....:*.:::*==+==+====:..==-...........................:.:.                                               ",
        "                                       :.....--===.....#.....-*#+#%*-+==+====..-==...=....................*.......-=                                             ",
        "                                      ..:=............:.......==+======+====+..==*...:......-.........:-...............                                          ",
        "                                      .:........:*.===:......:-.=+===+======..===:..........:.......:................-.=                                         ",
        "                                     ............=:.+==......+..==.========*..==:...+............:.===--..............-:=                                        ",
        "                                    .........-====+...+=.....=....==.=:====...=:...=......:===.==...........:.........=..:                                       ",
        "                                    -......=========*:...======..-=-.-...=...+:...+.....:==+.-=................-......==....                                     ",
        "                                    ......==========..:=...:+======-:...::.==..:==...-==:..-=+..................=:...-==...=                                     ",
        "                                     :...-==+=====...=.........:===-:..:.========++....-=*===+.==.................=..==+...=:                                    ",
        "                                     :..====:=====+*+........:...:+=...==-.........--....:==+====:-................-===..:===                                    ",
        "                                      -====-.=====-**-*+:.....-+:..=+.....:=.........:=+....=+===-.-....................====                                     ",
        "                                     ::====.======.**+*.**+=+=-:-=====.=+............=.....:+=====-.-...............:======-                                     ",
        "                                     -===+======*=.:.:.#*#*.*+..+*==*+*++*++.=++*===-.-=..-=.======..............:========+                                      ",
        "                                    ......:===+===.....#....-#**=====:..*#.+**++++-:-:...=..:======-...-==:....===========:.:                                    ",
        "                                    ........-=====-............=**+***##=.=-.*+.....:-..+...=========.....================.....                                  ",
        "                                   .........==-.===..............#%*=........:-...........=============-:....+============.....:.                                ",
        "                                   ........-=+.====+.............#%#:...................=======+=========+=====::-========.......=                               ",
        "                                    .....====-======+.........:=====-.................========*===+====++:========--::-===:.......+                              ",
        "                                  .....=====+========:..........==+===..............==========-.=========.=+=.............::......-=                             ",
        "                                 ..::.==============  .:=:.......=+=-...........:==========+=..=====+====..=-...............+.....:=.                            ",
        "                                 ..=..=============    :..==========+.......:============+================..+...............=+.....=+                            ",
        "                                :.:-...:=--=+==+:      .:=+....:-====+*===============+=-:.==========+====..................:==....==                            ",
        "                                ..=*......====+==        ++=......:=+-.....:-==========...=======+======:==..=...............===..-==                            ",
        "                               ...=-.......-=====          +*.....-=+.........:====+===-======++=======--===..:..............=======*.                           ",
        "                               ...=........===+==           ==*=======...:============:====-==========-.==== ................==*====-.:                          ",
        "                               ...+......:====+==           -....:===+==========++====+===============..-==+  =..............======-..=.                         ",
        "                                ..+.....======*=            +......==+.......::=========+=============...===   =.-...........====-....:-                         ",
        "                                ..+....========+           :-==...-==+.....-=========+===============+-+==++     ====-......=====.....==+                        ",
        "                                ..=...=======*=          :--+..===+*+==-=========*========+:=========*...==.      -======.:===*=:..::..==                        ",
        "                                ..-..-========.         ::--=......==========+=====*======:-=..======:...==         ===.=======...=+...--+                       ",
        "                                .....========:        .-:#######%##*+...........-=====*=.:===:..=====....==            =-.==:....=+=...-+.+                      ",
        "                               ......======+=         :-#%%*#######%##=.......-===+===..====+..:=====....=+             =-.......+=:...=+.=                      ",
        "                               %.....-====++         -:::%%#*#########%%#====++=====.:======...=====:....=:            :........===....====.                     ",
        "                               +=.....====+          ::::#%%########%%#%%....:====+===-==+=:..======+=...=            -........===-....====.                     ",
        "                               +=+=+**###*:         --:::-=%%*#######%%%%%:========..-=+==:.:*-::-:-+--:.=           ..........=*=:...-====                      ",
        "                              :+=+==+*###*         .:-::::=-=##**#####%%%%%=-::::::-====-:::::-==+++=+++:+           :.........=+=...===+=.                      ",
        "                              ++=+==+#*##          -=:+::::=:-::::::-::-:-:-=:::::::::::::-+=+=+=+==+++++-           :.........==:..=====+                       ",
        "                             +=+++==+***#          +::::::=::::::::::::-:::::::::::::::::::-+=-:-=+==+=++            =.........==..:=====+                       ",
        "                          .@%##=+==+=#**%          ==::::-:::::::::::::+:::::::::::::::::::-:--+=+++=+++=            =........===..======-                       ",
        "                          *%##**#==*==+##%         -:=:::=:::::::::::::+:::::::::::::::-=+++++++==++++++:            :........===.:======                        ",
        "                            #%%%%%%#*###%%*         =-=::::::::::::::::-::-::-:::::::::---:::-+++==+++++-             .......:==:.=======                        ",
        "                            -+***=###%#*.           %#:==:::::::::::::--:--:::=+=:::::::-=+++=+=*=++==+++             -......-==..=======                        ",
        "                            ++**+.++++===*         =###=+:::::::::::::-:::-:::::::=+++=++++++*++==+++++*              :.......==..======+                        ",
        "                           .++**++==*===+*%:       #####::::::::::::::--::-::::::===:---=+=+=+++=++=#%%%              ........==..======                         ",
        "                           -****+++=+==+*==*-     +#####::::::::::::::-:::-#---:::::::-:::-:::++%%%%%%%%*              :......-=..======                         ",
        "                           ++***++=+*==+...:.     *####%::::::::::::::-###########%%#**+===+#%%%%%%%%%%%%=             -.......=..=====+                         ",
        "                          .++**+++==+=:.....=     =####-::::::::::::::=##############%%%%%%%%%%%%%%%%%%%%%.             .......-..=====:                         ",
        "                          -+***+*+=++.......=+    %####:::::::::::::::-##############%%%%%%%%%%%%%%%%%%%%%%             -.........=====                          ",
        "                          +++*+=+=+*##  +-..==   #%####:::::::::::::::-#############%%%%%%%%%%%%%%%%%%%%%%#%            -........:=====                          ",
        "                          ++*++=*==##..:  -=+=   #####%:::::::::::::::=############%%%%%%%%%%%%%%%%%%%%%%%%%:            ........======                          ",
        "                         -++++==+=*##:#=   .==+ .#####*:::::::::::::::-###########%%%%%%%%%%%%%%%%%%%%%%%%%%%            -.......-====+                          ",
        "                         +=+=-.-...=       -=*= *#%###-:::::::::::::::=##########%%%%%%%%%%%%%%%%%%%%%%%%%%%%%           -.......:=====                          ",
        "                         ++--=.=..-:        -%%:*#####:::::::::::::::-#%########%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%+          -........====-                          ",
        "                        ---:.:==..=...        .  ###%#:::::::::::::::=%%%######%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%+           ........====-                          ",
        "                        =-:.: -.:-====.         =#####::::::::::::-::+%%%#######%%###%%%%%%%%%%%%%%%%%%%%%%%%%           *........====*@%%                       ",
        "                        -:..   .-.=+:-          #####+:::::::::::==::+%%%%###############%%%%%%%%%#%%%%%%%%%%%         %@@........===== +%%#                     ",
        "                       :.:.-   =-.-=:          ######=:::::::::::++::+%%%%#############%%%%%%######%%%%%%%%%%%:       %%@-........=====- +%%                     ",
        "                       ...      .-===:        +######-:::::::::::++::=%%%%%#########%%%%%%%%####%%%%%%%%%%%%%%#       +%%#:......=====*%%#%                      ",
        "                       .. :      ..%=-        #######::::::::::::++::=%%%%%##%%%%##%%%%%%%%%%%%%%%%%%%%%%%%%%%%      .:.*%%%%%%%%%%%%%%%=                        ",
        "                      -.                     ########::::::::::::++::+%%%%%%%%%%%##%%%%%%%%%%%%%%%%%%%%%%%%%%%%=    :..........==========                        ",
        "                      :...                 .#########::::::::::::++::+%%%%%%%%%%%##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   ............=========+                        ",
        "                     . . .                +##########::::::::::::==::+%%%%%%%%%%#####%%%%%%%%%%%%%%%%%%%%%%%%%%%  :............=========+                        ",
        "                     :                   %###########:::::+=+::::++::+%%%%%%%%######%%%%%%%%%%%%%%%%%%%%%%%%%%%%%=.............==========:                       ",
        "                     :  -               #############:::::+++-::-=+::+%%%%%%########%########%%%%%%%%%%%%%%%%%%%%.............===========*                       ",
        "                        .              %#############::::-+++:::==+::*%%%%%%%#######%%##########%%%%%%%%%%%%%%%%%......====-..===+=======*                       ",
        "                    :  -              =#############=::::=+=+:::=++:-#%%%%#%%%#############%%%%%%%%%%%%%%%%%%%%%..+=#.:==:.=..===+======+:                       ",
        "                    :  :              ##############::::++++=:::+++:=%%%%%%%% =%%#########%%%%%%%%%%%%%%%%%%%%%#+.=+##.....=.===========                         ",
        "                    - -              =##############::::==++-:::++=:=%%%%%%##   %#########%%%%%%%%%%%%%%%%%%%%%##--#%%%-:..=.=======+===                         ",
        "                     ..              -#############*:::-+==+-:::++=:+%%%%%%%%    ###########%%%%%%%%%%%%%%%%%%%%%##%%%%%*....===-===-==+                         ",
        "                   : :               *#############::-:+++=+=:::=++:+%%%%%%%%    ####################%%%%%%%%####%%%%#%%#.-.=*:.:===.===                         ",
        "                   :.                -#############--::=+++=-:::-++:=%%%%%%%#   .###################%%#########%%%%##%%%#..==...:==+===                          ",
        "                   :-                 ##############:-::::=+:::::++=-#%%%%%%#   *############################%%%####%%%%%.:== =..=*-                             ",
        "                   .                  *###############*-:::::::::-+=:+%%%%%%%   ############################%%%####%%%%%*=*==...==-                              ",
        "                  .-                   #############%%%%%%::::::::+=-+%%%%%%%#  ##########################%%%#####%%%%%%+:==%..:==                               ",
        "                  -                    -###########%%%%%%%%%*-::::-=++%%%%%%%%  #########################%%######%%%%%%%.+=*..-=+                                ",
        "                  .                     -#########%%%%%%%%%%%%%%#::++%%%%%%%%   +#########%%%%##################%%%%%%%%%%%%+%%                                  ",
        "                                          +######%%%%%%%%%%%%%%%%%%%%%%%%%%:     #############################%%%%%%%%%%%%%%%=                                   ",
        "                                            ####%%%%%%%%%%%%%%%%%%%%%%%%++       %###########################%%%%%%%%%%%%%%%#                                    ",
        "                                              #%%%%%%%%%%%%%%%%%%#%%=====        *##########################%%%%%%%%%%%%%%%=                                     ",
        "                                                :*%%%%##=-%%#%#=========:         ########################%%%%%%%%%%%%%%%%-                                      ",
        "                                                       ....========+====          +######################%%%%%%%%%%%%%%%#                                        ",
        "                                                        ..:============            %###################%%%%%%%%%%%%%%%%                                          ",
        "                                                        :..===========-             %#################%%%%%%%%%%%%%%-                                            ",
        "                                                         -:.==========%               %%%%%#####%%%%%%%%%%%%%%%%%*                                               ",
        "                                                        =%.:.========*%-                #%%%%%%%%%%%%%%%%%%%%%%-                                                 ",
        "                                                         %%:.=======#%%:                   .-=*%%%%%%%%%%%%*===-                                                 ",
        "                                                         -%%%@#*+%%%%===                        -.....:========.                                                 ",
        "                                                        -....+*%%=:=+===                        :.....:========                                                  ",
        "                                                     :.......-=.:=======                         -.....:=======                                                  ",
        "                                                  :..........:=========-                         .+.....=======                                                  ",
        "                                               .................=======                           -....========                                                  ",
        "                                          ..:...............-=========                             =...=======:                                                  ",
        "                                         #.-.............==========-                               #....======#=                                                 ",
        "                                        #:.=...........==:.                                       %@:...:======%%                                                ",
        "                                        #:.=.........-=                                          :%#:...::.====:%%                                               ",
        "                                        =#.=:......:=-                                            %%:...-..====*%#                                               ",
        "                                         *##...:-==.                                               *%%@==..-=#%%#                                                ",
        "                                          :=-+.                                                    :..+@%%%%@#===                                                ",
        "                                                                                                    ::=.....:===:                                                ",
        "                                                                                                    :=......-===                                                 ",
        "                                                                                                   ............=+                                                ",
        "                                                                                                   :............-:                                               ",
        "                                                                                                    .............-                                               ",
        "                                                                                                    ..............=                                              ",
        "                                                                                                     :..............                                             ",
        "                                                                                                     :..............                                             ",
        "                                                                                                     ...............+                                            ",
        "                                                                                                    .................                                            ",
        "                                                                                                    :................-                                           ",
        "                                                                                                     :....:..-..-..:..                                           ",
        "                                                                                                      :....-.::..--:.=                                           ",
        "                                                                                                      -.:##=..-=#=                                               ",
        "                                                                                                       ..:.+.:=                                                  "
        };
        aparenciaReduzida = Aparencia::reduzirEscalaAscii(aparenciaOriginal);
    }
    return aparenciaReduzida;
}

// --- BESTIARIO E DROPS ---
InfoBestiario Maho::obterInfoBestiario() const {
    return {
        "Labirinto Subterraneo", 
        "Sala do Chefe",
        "Uma entidade lendaria de oito empunhaduras selada na escuridao. A roda sobre sua cabeca gira, adaptando-se a todos e quaisquer fenomenos.",
        "Dizem que o unico jeito de derrota-lo e com um golpe fulminante antes que a roda gire.",
        {"Ouro Massivo", "Espada de Exterminio", "Roda da Adaptacao", FabricaItens::obterNomeDeID(ItemID::PedraUpgrade), FabricaItens::obterNomeDeID(ItemID::PocaoFuria), FabricaItens::obterNomeDeID(ItemID::ElixirArcano)},
        10 // Dificuldade Maxima!
    };
}

void Maho::realizarDrops(SistemaPersonagem* inimigo, SistemaPersonagem* jogadorAtual, std::vector<std::string>& itensObtidos, int& ouroTotal, int& xpTotal) {
    GerenciadorDrops::relatarEProcessarXpOuro(jogadorAtual, 5000, 5000, ouroTotal, xpTotal);
    
    GerenciadorDrops::darEProcessarItem(jogadorAtual, ItemID::EspadaExterminio, 1, itensObtidos);
    GerenciadorDrops::darEProcessarItem(jogadorAtual, ItemID::RodaAdaptacao, 1, itensObtidos);
    GerenciadorDrops::darEProcessarItem(jogadorAtual, ItemID::PedraUpgrade, 3, itensObtidos);
    GerenciadorDrops::darEProcessarItem(jogadorAtual, ItemID::PocaoFuria, 3, itensObtidos);
    GerenciadorDrops::darEProcessarItem(jogadorAtual, ItemID::ElixirArcano, 3, itensObtidos);
}
