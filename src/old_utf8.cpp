    ...
    if (flags.isLabirinto) {
        ...
        if (ty >= 50 && ty <= 60) {
            ...
            if (std::abs(tx - posCogumeloX) < 3) {
                if (ty < 54) { // Chap├⌐u do cogumelo
                    baseR = 150; baseG = 30; baseB = 150; // Roxo m├¡stico
        ...
        if (charParede == '|') {
            ...
            if (flags.tituloUpper.find("IGREJA") != std::string::npos) {
                // Vitrais geom├⌐tricos coloridos da igreja
                ...
                if (isVitral) {
                    ...
                    baseR = 210; baseG = 205; baseB = 195; // M├írmore claro
                }
            }
            ...
            // Outros mapas (PATIO DO REINO, Vila, etc.)
                ...
                // Em volta do Alquimista ('Q') -> Prateleiras de po├º├╡es
                if (npcEncontrado == 'Q') {
                ...
                // Em volta da Igreja ('I' ou 'P') -> Entrada da Igreja (M├írmore e porta de madeira com arco)
                else if (npcEncontrado == 'I' || npcEncontrado == 'P') {
                    // Pilares de m├írmore nas bordas
                    if (tx < 8 || tx > 55) {
                    ...
                    // Arco de pedra sobre a porta
                        ...
                        if (arcX*arcX + arcY*arcY > 1.0f) {
                            ...
                            // Vitral acima da porta (meio c├¡rculo)
                    ...
                    // Porta dupla de madeira pesada
                        ...
                        // Dobradi├ºas/ma├ºanetas de ferro/ouro
                ...
                // Anok ('A') -> Manequins detalhados com manto degrad├¬ e pedestal (Mantido perfeito)
                else if (npcEncontrado == 'A') {
                    // Fundo de madeira elegante do ateli├¬
                    ...
                    // Cabe├ºa do manequim (elipsoide no topo central)
                    ...
                    // Haste preta/met├ílica do pedestal
                    ...
                    if (isPrateleira) {
                        ...
                        if (tx % 4 == 0) { baseR = 200; baseG = 180; baseB = 120; } // P├íginas/Detalhes
                    } else if (ty > 32 && ty < 40 && (tx % 12 > 2 && tx % 12 < 10)) { // Sacos de ouro/itens
                        baseR = 160; baseG = 140; baseB = 100; // Tecido cru
                        if (ty < 35) { baseR = 100; baseG = 80; baseB = 50; } // Corda de amarra├º├úo
                    } else if (ty > 48 && ty < 56 && (tx % 20 > 4 && tx % 20 < 16)) { // Ba├║s pequenos
                        baseR = 100; baseG = 60; baseB = 20; // Madeira
                        if (tx % 20 < 6 || tx % 20 > 14 || ty == 52) { baseR = 80; baseG = 80; baseB = 80; } // Ferro do ba├║
                    } else {
                ...
                // Bjorn ('B') -> Fundo com bigorna, espadas, machados e arcos
                    ...
                    // Fundo da parede (madeira r├║stica)
                    ...
                    if (tx >= 8 && tx <= 12 && ty >= 10 && ty <= 40) {
                        if (tx == 10 && ty >= 10 && ty <= 30) { baseR = 190; baseG = 195; baseB = 200; } // L├ómina
                    ...
                    // Machado na direita (tx=52 a 58, ty=15 a 45)
                        ...
                        if (tx == 55 && ty >= 15 && ty <= 45) { baseR = 80; baseG = 50; baseB = 20; } // Cabo de madeira
                            ...
                            if (tx < 55) { baseR = 180; baseG = 180; baseB = 185; } // L├ómina do machado
                        }
                    }
...
Pixel3D RaycasterMundo::obterPixelChao(const std::string& tituloMapa, float currentX, float currentY, float currentDist, float profundidadeMaxima, const std::vector<std::tuple<int, int, int>>& luzes, const std::vector<std::string>* matrizDoMapa, float tempoAnimacao) {
    ...
    if (isLabirinto) {
        ...
        // Ch├úo de musgo e terra para Coracao da Floresta
...
char RaycasterMundo::obterSpriteChar(int /*mapX*/, int mapY, char c, const std::string& tituloMapa) {
    ...
    // Customiza├º├╡es para o PATIO DO REINO (e REINO) e Igreja
