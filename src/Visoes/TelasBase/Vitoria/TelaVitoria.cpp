...
public:
    CenaVitoriaIntro3D(Personagem* jogador, int ouro, int danoCausado, int danoRecebido, int cura, int turnos)
        : jogadorAtual(jogador), quantidadeDeOuroObtido(ouro), totalDeDanoCausado(danoCausado),
          totalDeDanoRecebido(danoRecebido), curaTotalRecebida(cura), turnosCombate(turnos) {
...
protected:
    ...
    std::vector<std::string> comporFundoComElementos(float opacity) const override {
        ...
        // Frame 0 logic to get initial state of boxes
...
void TelaVitoria::exibir(Personagem* jogadorAtual, int quantidadeDeOuroObtido, int quantidadeDeXpObtido, int totalDeDanoCausado, int totalDeDanoRecebido, int curaTotalRecebida, int turnosCombate, const std::vector<std::string>& itensObtidos)
{
    ...
    if (TelaCombate::isModo3D) {
        CenaVitoriaIntro3D intro(jogadorAtual, quantidadeDeOuroObtido, totalDeDanoCausado, totalDeDanoRecebido, curaTotalRecebida, turnosCombate);
        intro.executar();
    }
    ...
    for (int frame = 0; frame <= framesTotal; ++frame) {
        ...
        if (!TelaCombate::isModo3D) std::cout << "\033[J";
        std::cout << std::flush;
