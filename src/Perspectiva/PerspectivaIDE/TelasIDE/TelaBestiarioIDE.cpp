...
#include "../../TelasBase/Bestiario/TelaBestiarioLayout.h"

namespace {
    void exibirCabecalho(int largura, const std::string& tituloSecundario = "", bool animar = false) {
        Aparencia::limparTela();
        Aparencia::exibirPainelArte(ArtesBestiario::logoBestiario, 101, Cor::VERDE, tituloSecundario, animar);
    }
}

void TelaBestiarioIDE::exibirLista(Personagem* jogadorAtual) {
...
void TelaBestiarioIDE::exibirFicha(Personagem* jogadorAtual, const std::string& nomeSelecionado, int /*indiceDescoberto*/, const std::vector<std::string>& /*descobertos*/) {
    if (jogadorAtual == nullptr) return;
    ...
    int vezesDerrotado = bestiario.obterQuantidadeDerrotas(nomeSelecionado);
