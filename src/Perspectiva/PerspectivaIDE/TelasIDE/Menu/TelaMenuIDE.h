...
public:
    ...
    static int exibirOpcoesMenuPrincipal();
    static bool exibirConfirmacaoSaida();
    
    // Novas funcoes de utilidade
    static std::vector<std::string> comprimirArteASCII(const std::vector<std::string>& arteOriginal, int fatorY, int fatorX);
    static void imprimirBlocoCentralizadoIDE(const std::vector<std::string>& bloco, int recuoAdicionalX = 0);
    static int calcularEspacoY(int linhasDoConteudo);
};
