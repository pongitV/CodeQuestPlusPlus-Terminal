...
void TelaCombateIDE::definirTurnoVisivel(int turno, const std::string& nome) {
    TelaCombate::turnoAtualVisivel = turno;
    TelaCombate::nomeTurnoVisivel = nome;
}
...
void TelaCombateIDE::configurarContexto3D(bool modo3D, const std::vector<std::string>& matriz, float posX, float posY, float angulo, const std::string& titulo) {
    TelaCombate::isModo3D = modo3D;
    TelaCombate::matrizDoMapaAtual = matriz;
    TelaCombate::jogadorPosX = posX;
    TelaCombate::jogadorPosY = posY;
    TelaCombate::jogadorAngulo = angulo;
    TelaCombate::tituloMapaAtual = titulo;
}
...
void TelaCombateIDE::animarIntroducaoCombate(const std::string& titulo, const std::vector<Personagem*>& inimigos, Personagem* jogadorAtual) {
    ...
    if (TelaCombate::isModo3D && jogadorAtual != nullptr) {
        int larguraTerminal = Aparencia::obterLarguraTerminal();
        int alturaTerminal = Aparencia::obterAlturaTerminal();
...
int TelaCombateIDE::obterAcaoDoJogador(int turnoAtual, Personagem* personagemAgindo, const std::vector<Personagem*>& inimigos, Personagem* jogadorAtual, const std::vector<Personagem*>& aliados) {
    ...
    TelaCombate::personagemHUD = personagemAgindo;
    TelaCombate::selecaoAcaoAtual = 0;
    ...
    auto construirOpcoes = [&]() {
        TelaCombate::opcoesMenuAtual.clear();
        if (TelaCombate::personagemHUD != nullptr && TelaCombate::personagemHUD != personagemAgindo) {
            TelaCombate::opcoesMenuAtual.push_back("Voltar a Acao");
            TelaCombate::opcoesMenuAtual.push_back("Ver Aliados");
        } else {
            TelaCombate::opcoesMenuAtual.push_back("Atacar");
            TelaCombate::opcoesMenuAtual.push_back("Defender");
            ...
            TelaCombate::opcoesMenuAtual.push_back(habOption);
            ...
            if (temInventario || personagemAgindo == jogadorAtual) {
                TelaCombate::opcoesMenuAtual.push_back("Inventario");
            }
            
            TelaCombate::opcoesMenuAtual.push_back("Ficha");
            if (!aliados.empty()) TelaCombate::opcoesMenuAtual.push_back("Ver Aliados");
            TelaCombate::opcoesMenuAtual.push_back("Diario");
            TelaCombate::opcoesMenuAtual.push_back("Log Batalha");
        }
    };
    ...
    while (true) {
        ...
        if (ControleDeInput::teclaPressionada()) {
            ...
            int totalOpcoes = static_cast<int>(TelaCombate::opcoesMenuAtual.size());

            if (tecla == 'w' || tecla == 'W') { 
                TelaCombate::selecaoAcaoAtual--; 
                if (TelaCombate::selecaoAcaoAtual < 0) TelaCombate::selecaoAcaoAtual = totalOpcoes - 1; 
                ultimoUpdate = std::chrono::steady_clock::now();
                atualizarTelaEstatica("", inimigos, jogadorAtual, aliados);
            }
            else if (tecla == 's' || tecla == 'S') { 
                TelaCombate::selecaoAcaoAtual++; 
                if (TelaCombate::selecaoAcaoAtual >= totalOpcoes) TelaCombate::selecaoAcaoAtual = 0; 
                ultimoUpdate = std::chrono::steady_clock::now();
                atualizarTelaEstatica("", inimigos, jogadorAtual, aliados);
            }
            else if (tecla == '\r' || tecla == '\n') { 
                std::string op = TelaCombate::opcoesMenuAtual[TelaCombate::selecaoAcaoAtual];
                if (op == "Ver Aliados") {
                ...
                TelaCombate::selecaoAcaoAtual = -1; // Remove cursor para as animacoes manterem a tela limpa
                TelaCombate::personagemHUD = nullptr; // Garante que o HUD volte a mostrar o personagem agindo de fato nas proximas animacoes
                atualizarTelaEstatica("", inimigos, jogadorAtual, aliados);
...
int TelaCombateIDE::obterAlvoAtaque(const std::string& tituloCombate, const std::vector<Personagem*>& inimigos, Personagem* jogadorAtual, const std::vector<Personagem*>& aliados) {
    ...
    TelaCombate::selecaoAlvoAtual = 0;
    int totalInimigos = static_cast<int>(inimigos.size());
    int oldAcao = TelaCombate::selecaoAcaoAtual;
    TelaCombate::selecaoAcaoAtual = -1; // Esconde o cursor do menu de acoes

    TelaCombate::piscarSelecao = true;
    ...
    while (true) {
        ...
        if (std::chrono::duration_cast<std::chrono::milliseconds>(agora - tempoBlink).count() >= 150) {
            TelaCombate::piscarSelecao = !TelaCombate::piscarSelecao;
            tempoBlink = agora;
        }
        ...
        if (ControleDeInput::teclaPressionada()) {
            ...
            if (tecla == 'a' || tecla == 'A') { 
                TelaCombate::selecaoAlvoAtual--; 
                if (TelaCombate::selecaoAlvoAtual < 0) TelaCombate::selecaoAlvoAtual = totalInimigos - 1; 
                TelaCombate::piscarSelecao = true;
                ...
                TelaCombate::selecaoAlvoAtual++; 
                if (TelaCombate::selecaoAlvoAtual >= totalInimigos) TelaCombate::selecaoAlvoAtual = 0; 
                TelaCombate::piscarSelecao = true;
                tempoBlink = std::chrono::steady_clock::now();
                atualizarTelaEstatica(tituloCombate, inimigos, jogadorAtual, aliados);
            }
            else if (tecla == '\r' || tecla == '\n') { 
                int escolha = TelaCombate::selecaoAlvoAtual;
                TelaCombate::selecaoAlvoAtual = -1; 
                TelaCombate::selecaoAcaoAtual = oldAcao;
                TelaCombate::piscarSelecao = true;
                atualizarTelaEstatica(tituloCombate, inimigos, jogadorAtual, aliados);
                return escolha; 
            }
...
void TelaCombateIDE::selecionarHUDDeAliado(Personagem* jogadorAtual, const std::vector<Personagem*>& aliados) {
    ...
    if (escolha >= 0 && escolha < static_cast<int>(opcoesHUD.size()) - 1) {
        if (escolha == 0) TelaCombate::personagemHUD = jogadorAtual;
        else TelaCombate::personagemHUD = aliados[escolha - 1];
    }
}
...
int TelaCombateIDE::obterEscolhaDeEscudo(const std::string& nomePersonagem, const std::vector<Item*>& listaDeEscudos) {
    if (TelaCombate::isTerminalView) {
        int escolha = TelaCombateIDE::exibirSelecaoEscudo(TelaCombate::personagemHUD, listaDeEscudos);
...
void TelaCombateIDE::animarMorteInimigo(const std::string& tituloCombate, const std::vector<Personagem*>& listaDeInimigos, Personagem* inimigoMorto, Personagem* jogadorAtual, const std::vector<Personagem*>& listaDeAliados, const std::vector<std::string>& drops)
{
    ...
    if (TelaCombate::isTerminalView) {
        TelaCombate::g_inimigoMortoComDrops = inimigoMorto;
        TelaCombate::g_dropsAtivos = drops;
