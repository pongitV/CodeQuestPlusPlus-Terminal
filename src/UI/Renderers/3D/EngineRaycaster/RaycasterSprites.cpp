...
SpriteCache RaycasterSprites::parsiSprite(const std::vector<std::string>& raw, int r, int g, int b, bool isMahoraga) {
    ...
    for (int y = 0; y < compHeight; ++y) {
        for (int x = 0; x < compWidth; ++x) {
            ...
            bestPixel.isTransparent = true;
            for(int dy = 0; dy < scale; ++dy) {
                for(int dx = 0; dx < scale; ++dx) {
                    int oy = y * scale + dy;
                    int ox = x * scale + dx;
                    if (oy < sc.height && ox < sc.width) {
                        if (!tempPixels[oy][ox].isTransparent) {
    ...
    SpriteCache endSc;
    endSc.width = compWidth;
    endSc.height = compHeight;
    endSc.pixels.resize(compWidth * compHeight);
    
    for (int y = 0; y < compHeight; ++y) {
        for (int x = 0; x < compWidth; ++x) {
            if (compressedTempPixels[y][x].isTransparent) {
                SpritePixel sp;
                sp.isTransparent = true;
                endSc.pixels[y * compWidth + x] = sp;
            } else {
                bool isEdge = false;
                for (int dy = -1; dy <= 1; ++dy) {
                    for (int dx = -1; dx <= 1; ++dx) {
                        ...
                        else if (compressedTempPixels[ny][nx].isTransparent) isEdge = true;
                    }
                }
                if (isEdge) {
                    SpritePixel sp;
                    ...
                    endSc.pixels[y * compWidth + x] = compressedTempPixels[y][x];
                }
            }
        }
    }
...
std::vector<std::string> RaycasterSprites::colorArt(const std::vector<std::string>& art, const std::string& colorAnsi) {
    std::vector<std::string> artColorful;
    for (const auto& line : art) artColorful.push_back(colorAnsi + line + "\033[0m");
    return artColorful;
}
