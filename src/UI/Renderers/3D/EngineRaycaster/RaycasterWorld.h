...
public:
    ...
    static Pixel3D getInternalWallPixel(const std::string& titleMap, bool themeForest, float distanceUntilWall, float depthMaximum, char charWall, int y, int ceiling, int chao, float texX, float timeAnimation, const std::vector<std::tuple<int, int, int>>& lights, float hitX, float hitY, bool isSideWall = false, char npcFound = ' ', float nx = 0.0f, float ny = 0.0f);
    static Pixel3D getPixelWall(const std::string& titleMap, bool themeForest, float distanceUntilWall, float depthMaximum, char charWall, int y, int ceiling, int chao, float texX, float timeAnimation = 0.0f, bool isSideWall = false, const std::vector<std::tuple<int, int, int>>& lights = {}, float hitX = 0.0f, float hitY = 0.0f, char npcFound = ' ', float nx = 0.0f, float ny = 0.0f);

    static Pixel3D getInternalWallPixel(const std::string& titleMap, bool themeForest, float distanceUntilWall, float depthMaximum, char charWall, int y, int ceiling, int chao, float texX, float timeAnimation, const Highlighter::InfoLight& infoLight, float hitX, float hitY, bool isSideWall = false, char npcFound = ' ', float nx = 0.0f, float ny = 0.0f);
    ...
    static Pixel3D getPixelChao(const std::string& titleMap, float currentX, float currentY, float currentDist, float depthMaximum, const std::vector<std::tuple<int, int, int>>& lights = {}, const std::vector<std::string>* mapMatrix = nullptr, float timeAnimation = 0.0f);
    static Pixel3D getPixelChao(const std::string& titleMap, float currentX, float currentY, float currentDist, float depthMaximum, const Highlighter::InfoLight& infoLight);
