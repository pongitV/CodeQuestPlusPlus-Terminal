...
#include "../Visoes/TelasBase/Pause/TelaPause.h"
#include "../Core/Utilidades/Aparencia.h"
#include "../Visoes/TelasBase/MapaMundial/TelaMapaMundial.h"
...
#include "../Visoes/Raycaster/Engine3D/RaycasterMundo.h"
...
bool ControleMapa::isExploracao3DAtiva() { return GerenciadorVisao::obterInstancia().isVisao3DAtiva(); }
    ...
    int posicaoXAposCombate, int posicaoYAposCombate, int posicaoXInicialDoInimigo, int quantidadeDeCelulasOcupadas, int /*larguraDoTerminal*/, const std::function<void()>& restaurarTela)
{
    ...
    if (exploracaoEstaAtiva && !GerenciadorVisao::obterInstancia().isVisao3DAtiva()) restaurarTela();
}
    ...
    const std::vector<std::string>& matrizDoSubMapaGerada, int posicaoXInicialNoSubMapa, int posicaoYInicialNoSubMapa, const std::string& tituloDoSubMapa, const std::function<void()>& restaurarTela)
{
    ...
    if (!GerenciadorVisao::obterInstancia().isVisao3DAtiva()) restaurarTela();
}
...
std::string ControleMapa::formatarCelula(char celula, int x, int y, const std::string& tituloDoMapa, const std::vector<std::string>& matrizDoMapa, bool isMinimapa) {
    ...
    bool isEngineIDE = !isMinimapa && !GerenciadorVisao::obterInstancia().isVisao3DAtiva();
    if (isEngineIDE) {
...
ProximaTransicaoMapa ControleMapa::executarLoopDeExploracao(
    ...
    while (exploracaoEstaAtiva && jogadorAtual->obterVida() > 0)
    {
        ...
        if (precisaRenderizar && !GerenciadorVisao::obterInstancia().isVisao3DAtiva()) {
        ...
        bool processarInput = false;
            ...
            if (!GerenciadorVisao::obterInstancia().isVisao3DAtiva() && ControleDeInput::teclaPressionada()) {
            teclaPressionadaPeloJogador = ControleDeInput::lerTecla();
            processarInput = true;
        }

        if (GerenciadorVisao::obterInstancia().isVisao3DAtiva() || (processarInput && (teclaPressionadaPeloJogador == 'v' || teclaPressionadaPeloJogador == 'V'))) {
            static std::string tituloAnterior = "";
            int tipoAnimacao = 0;
            
            bool trocandoDePerspectiva = !GerenciadorVisao::obterInstancia().isVisao3DAtiva();
            ...
            if (!GerenciadorVisao::obterInstancia().isVisao3DAtiva()) {
                GerenciadorVisao::obterInstancia().alternarVisao();
            }
                ...
                if (hitX != -1 && hitY != -1) {
                    ...
                    if (isTrigger && GerenciadorVisao::obterInstancia().isVisao3DAtiva() && posicaoXDoJogador == posXantes && posicaoYDoJogador == posYantes) {
