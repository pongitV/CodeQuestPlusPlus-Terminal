...
namespace {
    ...
    void rodarTutorialDigitacao(const std::string& infoBox) {
        ...
        std::vector<std::string> explicacao = {
            "═══ COMO FUNCIONA O PARRY DIGITACAO ═══",
    ...
    void rodarTutorialMovimento(const std::string& infoBox) {
        ...
        std::vector<std::string> explicacao = {
            "═══ COMO FUNCIONA O PARRY MOVIMENTO ═══",
...
int TelaMenu::exibirPromptDificuldade(const std::string& nome, const std::string& nomeRaca, const std::string& nomeClasse) {
    ...
    std::cout << "\033[?25l";
    Aparencia::limparTela();
    TelaMenu::exibirPainelLogoJogo("DIFICULDADE DO MUNDO", false);
    ...
    while (true) {
        ...
        if (!ControleDeInput::teclaPressionada()) {
            ...
            continue; // Continua o loop para manter a animação rodando
        }
        ...
        if (tecla == 224 || tecla == 0 || tecla == '\033') {
            ...
            else if (proxTecla == 27) return totalOpcoes - 1; // "ESC" cancela para o Voltar
        }
